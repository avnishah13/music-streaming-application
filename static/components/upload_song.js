	    ...
	    <form @submit.prevent="submit" action="/creator/song" method="POST" enctype="multipart/form-data">
	      <fieldset>
	        <div class="form-group">
	          ...
	          <input class="form-control" id="song_name" placeholder="Julia" name="song_name" style="width: 600px; margin: 10px auto;" required>
	        </div>
	        ...
	        <div class="form-group">
	          ...
	          <textarea class="form-control" name="lyrics" id="lyrics" style="width: 600px; margin: 10px auto; display:block; white-space: pre-wrap;"></textarea>
	        </div>
	        ...
	        <div class="form-group" id="duration" style="float:left; margin-left: 475px;">
	          ...
	          <input id="minutes" type="number" name="minutes" style="width: 200px; margin: 10px auto;" required>
	        </div>

	        <div class="form-group" style="float:left;">
	          <label for="seconds" class="form-label mt-4">&nbsp;&nbsp;&nbsp;&nbsp;Seconds</label>
	          <input id="seconds" name="seconds" type="number" max="60" style="width: 200px; margin: 10px auto;" required>
	        </div>
	        <div style="clear:both;">&nbsp;</div>

	        <div>
	          <label for="audio" class="form-label mt-4">Audio</label>
	          <input class="form-control" type="file" id="audio" name="audio" accept=".mp3" @change="AudioUploaded($event)" style="width: 600px; margin: 10px auto;" required>
	        </div>

	        <div class="form-group" v-if="counter>=1 && count >= 1">
	          <label for="creator_id_1" class="form-label mt-4">Collaborator Name</label>
	          <select class="form-select" name="creator_id_1" id="creator_id_1" @change="first_collab($event)" v-model="creator_id_1" style="width: 600px; margin: 10px auto;" required>
	            <option disabled selected value>Select Option</option>
	            <template v-for="creator in creators" :key="creator.creator_id">
	            <option v-if="creator.creator_id !== creator_id_2 && creator.creator_id !== creator_id_3 && creator.creator_id !== creator_id_4 && creator.creator_id !== creator_id_5" :value=creator.creator_id>{{creator.creator_name}}</option>
	          	</template>
	          </select>
	        </div>

	        <div class="form-group" v-if="counter>=2 && creator_id_1 != ''  && count >= 2">
	          ...
	          <select class="form-select" name="creator_id_2" id="creator_id_2" @change="onChange($event)" v-model="creator_id_2" style="width: 600px; margin: 10px auto;" required>
	            <option disabled selected value>Select Option</option>
	            <template v-for="creator in creators" :key="creator.creator_id">
	            <option v-if="creator.creator_id !== creator_id_1 && creator.creator_id !== creator_id_3 && creator.creator_id !== creator_id_4 && creator.creator_id !== creator_id_5" :value=creator.creator_id>{{creator.creator_name}}</option>
	          	</template>
	          </select>
	        </div>

	        <div class="form-group" v-if="counter>=3 && creator_id_2 != ''  && count >= 3">
	          ...
	          <select class="form-select" name="creator_id_3" id="creator_id_3" @change="onChange($event)" v-model="creator_id_3" style="width: 600px; margin: 10px auto;" required>
	            <option disabled selected value>Select Option</option>
	            <template v-for="creator in creators" :key="creator.creator_id">
	            <option v-if="creator.creator_id !== creator_id_1 && creator.creator_id !== creator_id_2 && creator.creator_id !== creator_id_4 && creator.creator_id !== creator_id_5" :value=creator.creator_id>{{creator.creator_name}}</option>
	          	</template>
	          </select>
	        </div>

	        <div class="form-group" v-if="counter>=4 && creator_id_3 != ''  && count >= 4">
	          ...
	          <select class="form-select" name="creator_id_4" id="creator_id_4" @change="onChange($event)" v-model="creator_id_4" style="width: 600px; margin: 10px auto;" required>
	            <option disabled selected value>Select Option</option>
	            <template v-for="creator in creators" :key="creator.creator_id">
	            <option v-if="creator.creator_id !== creator_id_1 && creator.creator_id !== creator_id_2 && creator.creator_id !== creator_id_3 && creator.creator_id !== creator_id_5" :value=creator.creator_id>{{creator.creator_name}}</option>
	          	</template>
	          </select>
	        </div><br>
	...
	methods: {
		async submit(){
			...
			if (result.status_code === 200) {
				router.push({
		    		name: 'creator_home',
		    		query: { message: "Song Uploaded Successfully!" },
	    		});
