...
const top_playlists = {
	...
	<table class="table table-hover">
		...
		<br v-if="songs.filter(song => song.flag === null).length == 0 && flag == false"><br v-if="songs.filter(song => song.flag === null).length == 0 && flag == false">
		<h3 v-if="songs.filter(song => song.flag === null).length == 0 && flag == false">No songs in this playlist yet. We'll add some soon!</h3>
	</table>
...
`,
	...
	async created() {
    	...
    	if (this.token && (this.role == 'User' || this.role == 'Creator')) {
			...
			const res = await fetch(`/api/user/top_playlists?${queryParams}`, {
		        method: 'GET',
		    });
		    const result = await res.json();
