...
const admin_search = {
	...
	<div v-if="songs.length == 0 && albums.length == 0 && creators.length == 0">
	<h1>No Results Found!</h1><br><br>
	</div>

	<div v-if="songs.length !== 0" class="row" style="margin-left: 0px;">
	<b><p style="font-size: 35px; margin-left: 18px; text-align: center;">Songs</p></b>
	...
	<b><p style="font-size: 35px; margin-left: 18px; text-align: center;">Albums</p></b>
	  <table class="table table-hover">
	    <tr>
	...
	<b><p style="font-size: 35px; margin-left: 18px; text-align: center;">Creators</p></b>
		<div class="column" v-for="creator in creators" :key="creator.creator_id">
	      &nbsp;&nbsp;<img :src="'../static/creator_profiles/' + creator.creator_id + '.jpeg'" @error="changeImageSrc" style="width:200px; height:200px; border-radius: 25px;">
	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-footer">
	        <form :action="'/creator/song/'+ song_id +'/delete'" method="POST" @submit.prevent="confirm_delete" id="create_album">
	...
	<div class="modal" tabindex="-1" role="dialog" id="deleteSong">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h5 class="modal-title">Confirm Deletion</h5>
	      ...
	      <div class="modal-footer">
	        <form :action="'/creator/song/'+ song_id +'/delete'" method="POST" @submit.prevent="song_delete" id="create_album">
	        <button type="submit" class="btn btn-primary">Save changes</button>
	        <button @click="closeModal($event)" type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
	        </form>
	      </div>
...
`,
	...
	methods: {
		...
		changeImageSrc() {
		    event.target.src = "../static/creator_profiles/default.jpeg";
		},
		...
		delete_song(song_id) {
	    	...
	    	const modal = document.getElementById('deleteSong');
	        // Show the modal
	        if (modal) {
	            modal.classList.add('show');
	            modal.style.display = 'block';
	...
	async created() {
    	...
    	if (this.token && (this.role == 'Admin')) {
			...
			const res = await fetch(`/api/admin/search?${queryParams}`, {
		        method: 'GET',
		    });
		    const result = await res.json();
